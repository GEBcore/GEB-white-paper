...
- **问题的规约**
    ...
    这个更高层次的**分叉问题$Fork(B_A, B_B)$** 无法由确定性规则裁决，因此必须被移交给下一个要素——**神谕图灵机**。
...
综上所述**，序数逻辑系统**的核心，是为系统增加了一个新的时间戳 $T_{n+1}$。挖矿这一耗散结构将无形的计算转化为了不可逆的热力学时间，并将神谕那概率性的、待定的选择，牢牢地**固化**在了由物理定律保证单向性的时间序列中。
...
GEB四要素中的“神谕图灵机**”**在工程上被实现为一个**由Agere节点（作为神谕）**构成的任务驱动网络。它负责为系统无法由确定性规则解决的问题，生成非确定性的“候选解”。
